// Notifications
Meteor.methods({
    "notifications.set_read": function (id) {
        // TODO - Connect to DB
    },
    "notifications.set_all_read": function (id) {
        // TODO - Connect to DB
    },
    "notifications.remove": function (id) {
        // TODO - Connect to DB
        // Simulate DB
        var _old_data = EqApp.sample_notifications.get();
        for (var key in _old_data) {
            if (_old_data[key].id === id) {
                _old_data.splice(key, 1);
            }
        }
        EqApp.sample_notifications.set(_old_data);
    },
    "notifications.remove_all": function () {
        // TODO - Connect to DB
        // Simulate DB
        EqApp.sample_notifications.set([]);
    }
});
...
//  Sample data
//---------------------------------------------------
EqApp.sample_notifications = new ReactiveVar([]);
EqApp.sample_notifications.set(
[{
    "id": 1,
