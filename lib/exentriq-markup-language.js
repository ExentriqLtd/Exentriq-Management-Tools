	...
	parse: function(statement){
		try{
			...
			if (regexpBoardDoubleQuoteResult !== null) {
				...
				clean = clean.replace(regexpBoardDoubleQuoteResult[1]+"\""+regexpBoardDoubleQuoteResult[2]+"\"", "");
			}
			else{
				var regexpBoardResult = regexpBoard.exec(statement);
				if (regexpBoardResult !== null) {
					result.project = regexpBoardResult[2].trim();
					clean = clean.replace(regexpBoardResult[1]+regexpBoardResult[2], "");
				}
			}
			...
			if (regexDaysResult !== null) {
				...
				clean = clean.replace(regexDaysResult[1]+regexDaysResult[2], "");
			} else {
				result.days = 0;
			}
			...
			if (regexHoursResult !== null) {
				...
				clean = clean.replace(regexHoursResult[1]+regexHoursResult[2], "");
			} else {
				result.hours = 0;
			}
			...
			if (regexMinutesResult !== null) {
				...
				clean = clean.replace(regexMinutesResult[1]+regexMinutesResult[2], "");
			} else {
				result.minutes = 0;
			}
