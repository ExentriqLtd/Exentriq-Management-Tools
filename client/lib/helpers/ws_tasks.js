...
(function (mtr, $) {
    ...
    _this.updateCollection = function () {
        Session.set("tasksUpdatedNum", Session.get("tasksUpdatedNum")+1);
    };
    ...
    _this.all = function (type) {
        ...
        var sort = {};
        if(Session.get("tasksFiltersOrderByPoints")){
            sort.points = -1;
        }
        else if(Session.get("tasksFiltersOrderByCreationDate")){
            sort.milestone = -1; // TODO - remplace for creation date
        }

        return NotifyTasks.find(filter, {sort: sort});
    };
    ...
    _this.set_complete = function (id, value, update_collection) {
        ...
        // Update UI
        var doc = NotifyTasks.findOne({id:data.id});
        var d = new Date();
        var closed_on = null;
        if(data.open === 'false'){
    ...
    _this.reset_all_filters = function () {
        Session.set("tasksFiltersOrderByPoints", false);
        Session.set("tasksFiltersOrderByCreationDate", false);
    };
    ...
    _this.ws_open = function (callback, data, doc) {
        Meteor.call('tasks.open', data, doc,
        function(error, result){
            if($.isFunction(callback)){callback(result, error);}
        });
    };
